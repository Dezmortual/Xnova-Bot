"""Flask web dashboard — no template folder needed (HTML embedded)."""
import json
import time
...
# Dashboard HTML (embedded so no templates folder is needed)
DASHBOARD_HTML = None
def _load_html():
    ...
    if os.path.exists(p):
        ...
        DASHBOARD_HTML = "<h1>X-NOVA running</h1><p>Dashboard template not found.</p>"
...
def create_app() -> Flask:
    ...
    def api_state():
        ...
        equity_curve = [
            {"t": 0, "eq": PORTFOLIO.initial_balance},
            {"t": time.time()*1000, "eq": round(PORTFOLIO.equity(price), 2)},
        ]
        return jsonify({
